...
# The transformation matrix of R is calculated by entering the left side of the transformation
    def calc_R(self, pitch, yaw, roll):
        a = np.radians(pitch)
        b = np.radians(yaw)
        c = np.radians(roll)
...
# K is the camera parameter
    ...
    def convert_(self, x, y):
        ...
        pixel_w = 1912
        pixel_h = 1160
        K = self.calc_K(fov_x, pixel_w, pixel_h)
        ...
        z = 3
        return(self.convert_uvz_to_xyz(x, y, z, R, t, K))




def main_func():
    trans = TwoDPointToThreeD()

    def on_EVENT_LBUTTONDOWN(event, x, y, flags, param):
        if event == cv2.EVENT_LBUTTONDOWN:
            t_ = trans.convert_(x, y)
            x3d = t_[0]
            y3d = t_[1]
            z3d = t_[2]
